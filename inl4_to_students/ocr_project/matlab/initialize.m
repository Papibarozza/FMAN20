...
S_feats = zeros(6, 300); % 7, since I use 7 features -- change appropriately!
for i = 1 : numel(S)
    S_feat = segment2features(imgaussfilt(double(S{i}),0.5));
    S_feats(:, i) = S_feat;
end
%%
...
imgnr = 1;
for j = 1:5:1000
    imdata = load([datadir '/im' int2str(imgnr) '.mat']);
    segs = imdata.S;
    seg_y = uint16(imdata.facit)-64
    assert(sum(seg_y == 0) == 0,'Wrong')
    for i = 1 : numel(seg_y)
        S_feat = segment2features(imgaussfilt(double(segs{i}),0.5));
        S_feats(:, j+i) = S_feat;
    end
    y = [y seg_y];
    imgnr = imgnr+1;
...
%imdata = load('./datasets/home1/im1.mat')
%imdata = load([datadir '/im' int2str(1) '.mat'])
%%
...
classification_data = class_train(S_feats, uint8(y));
